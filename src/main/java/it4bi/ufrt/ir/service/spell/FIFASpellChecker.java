...
public class FIFASpellChecker {
	 ...
	 private final String COMBINED_DICT = "C:\\Users\\KK\\Google Drive\\Eclipse Workspace\\IR-Local\\Dictionaries\\Combined Dict - ANSI.txt";
	 private final String FIFA_DICT = "C:\\Users\\KK\\Google Drive\\Eclipse Workspace\\IR-Local\\Dictionaries\\FIFA DWH Dictionary - ANSI.txt";
	  ...
	  */
	public QueryAutoCorrectionResult autoCorrectQuery(String searchQuery, boolean suggest, int numberOfSuggestions) throws IOException{
		...
		for(String tok: tokinz){
			...
			if(wr.isCorrected()){
				...
				// for each corrected word, replace the misspelled one by the suggested one
			correctedQuery = correctedQuery.replaceAll(wr.getOriginalWord().toLowerCase(),wr.getCorrectedWord().toLowerCase());
		...
		if(suggest&& isCorrected){
			...
			for(int i=0; i< numberOfSuggestions; i++){
				...
				//loop on all mistakes and get index number i of their suggestions and replace it in the original query
				for(SpellCheckerResult r: wordCorrections){
					...
					sugstion = sugstion.replaceAll(r.getOriginalWord().toLowerCase(), suggestedWord.toLowerCase());
				}
	...
	public  SpellCheckerResult checkSpellingAndSuggest (String word,int suggestionsNumber,boolean ingnoreCase ) {
	 ...
	 */
	public String [] rankWords (String word, String [] similarWords) throws IOException{
		
		//ideas, rank based on JARO_WINKLER_DISTANCE
		// or based on existence in our custom DWH dictionary plus matches vocab (winner, loser, final..etc)
